...
VaultInfo = NamedTuple('VaultInfo', [
    ('stark_public_key', str), ('asset_id', int), ('vault_id', int)])
...
class OnchainUtils:
    def __init__(
            self, node_url: str, funder_private_key: str, gas_price: Optional[int] = None) -> None:
        """
    ...
    def _aggregate_deposit_amounts_by_vault(
            self, deposit_requests: List[Dict[str, Any]]) -> Dict[VaultInfo, int]:
        """
    ...
    def _get_revert_reason(self, tx_hash: HexStr) -> str:
        ...
        if client_version.startswith('Geth'):
            trace = self.w3.manager.request_blocking(RPCEndpoint('debug_traceTransaction'), [
    ...
    def _transmit_tx(
            self, tx: ContractFunction, sender: Account,
            tx_args: Optional[TxParams] = None) -> HexStr:
        """
    ...
    def _get_deposit_balances(
            self, users_tokens_vaults: Iterable[VaultInfo]) -> Dict[VaultInfo, int]:
        """
        ...
        return {
            vault: self.main_contract.functions.getQuantizedDepositBalance(
                int(vault.stark_public_key, 16), vault.asset_id, vault.vault_id).call()
    ...
    async def _onchain_deposits(self, aggregated_user_token_vault: Dict[VaultInfo, int]):
        ...
        if not all(
                receipt is not None and receipt['status'] == 1
                for receipt in receipts_dict.values()):
            failed = {
                tx_hash: receipt
                for tx_hash, receipt in receipts_dict.items()
                if receipt is None or receipt['status'] != 1}
            raise TransactionFailed(
                f'Deposit(s) failed. Failed transaction receipts:\n{failed}')
