...
async def tx_dicts_to_add_tx_dicts(
        ...
        gateway_client: PlaygroundGatewayClient) -> List[Dict[str, Any]]:
    """
...
async def send_tx(
        gateway_client: PlaygroundGatewayClient,
...
async def play_onchain_txs(
        onchain_player: OnchainUtils, tx_dicts: List[Dict[str, Any]]):
    """
...
def run_main():
    ...
    parser.add_argument(
        '--txs', type=str, required=True,
        help='Path to JSON file containing transactions to run.')
    parser.add_argument(
        '--gateway_url', type=str, default=DEFAULT_GATEWAY_URL,
        help='URL of the StarkEx gateway.')
    parser.add_argument(
        '--node_url', type=str, required=True,
        help='URL of the node processing the Ropsten transactions (required for deposits).')
    parser.add_argument(
        '--funder_private_key', type=str, required=True,
        help='The private ETH key of a funded account (for on-chain transactions).')
    parser.add_argument(
        '--verbose', action='store_true', default=False,
        help='Verbosity turned on.')
    args = parser.parse_args()

    asyncio.run(main(
        txs_path=args.txs, gateway_url=args.gateway_url, node_url=args.node_url,
        funder_private_key=args.funder_private_key, verbose=args.verbose))
